        ...
        if(request.getParameter("password1").equals(request.getParameter("password2")))
            {
                ...
                new Model().dataup("update member set password='"+pwd+"' where email='"+email+"' ");
                ResultSet rs= new Model().data("select * from member where email='"+email+"' ");
                try{
                    if(rs.next()){
                        new Model().dataup("update login set password='"+pwd+"' where username='"+rs.getString("username")+"' ");
                        response.sendRedirect("login.jsp?try=3");
                        
                    }else{
                        response.sendRedirect("forget.jsp?try=1");
                  ...
                  <div class="panel-body">
                    ...
                    <form action="forget2.jsp" id="register-form" role="form" autocomplete="off" class="form" method="POST">
                        ...
                        <div class="form-group">
                          <input id="pwd1" name="password1" placeholder="Password" class="form-control" required="required" type="password">
                        </div>
                      <div class="form-group">
                          <input id="pwd2" name="password2" placeholder="Confirm Password" class="form-control" required="required" type="password">
                        </div>
                      <div class="form-group">
